                          ...
                          itemBuilder: (context, index) => Container(
                              ...
                              child: new GestureDetector(
                                onTap: () {
                                  ...
                                  Globalvireables.visitno = _journals[index]
                                          ['orederno']
                                      .toInt()
                                      .toString();
                                  Navigator.push(
...
*/
  ...
  void _refreshItems() async {
    ...
    if ((datecontroler.text == null || datecontroler.text == "") &&
        Globalvireables.date == "") {
      data = await SQLHelper.GetVisited();
    } else {
      if (Globalvireables.date.contains("20") &&
  ...
  fillvisited() async {
    ...
    print(
        "visites " + Globalvireables.VisitsListAPI + Globalvireables.username);
    http.Response response = await http.get(apiUrl);
    ...
    for (var i = 0; i < list.length; i++) {
      await SQLHelper.createvisit(
          list[i]["OrderNo"], list[i]["CustomerNameA"],
          list[i]["Tr_Data"], list[i]["Start_Time"]
      );
      if (i == list.length - 1) {}
    }
  }
